...
UINT        string_count;
    ...
    status = _gxe_display_language_table_get(display, 0, 0, 0);
    EXPECT_EQ(GX_SUCCESS, status);
#endif

    status = _gxe_display_language_table_get_ext(GX_NULL, &language_table, &language_count, &string_count);
    EXPECT_EQ(GX_PTR_ERROR, status);

    status = _gxe_display_language_table_get_ext(display, &language_table, &language_count, &string_count);
    EXPECT_EQ(GX_SUCCESS, status);
    ...
    status = _gxe_display_language_table_get_ext(display, GX_NULL, GX_NULL, GX_NULL);
    EXPECT_EQ(GX_SUCCESS, status);
