...
GX_UBYTE   *root_address = (GX_UBYTE *)0x6000000;
    ...
    status = _gxe_binres_language_table_load_ext(GX_NULL, &language_table);
    EXPECT_EQ(GX_PTR_ERROR, status);

    status = _gxe_binres_language_table_load_ext(root_address, GX_NULL);
    EXPECT_EQ(GX_PTR_ERROR, status);

    gx_system_memory_allocator_set(rotate_memory_allocate, GX_NULL);
    status = _gxe_binres_language_table_load_ext(root_address, &language_table);
