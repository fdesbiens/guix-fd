...
GX_RECTANGLE           size;
    ...
    status = _gxe_string_scroll_wheel_string_list_set_ext(GX_NULL, GX_NULL, 0);
    EXPECT_EQ(GX_PTR_ERROR, status);

    /* Call with invalid widget. */
    memset(&wheel, 0, sizeof(GX_STRING_SCROLL_WHEEL));
    status = _gxe_string_scroll_wheel_string_list_set_ext(&wheel, test_string_list_1, TEST_STRING_LIST_SIZE);
    EXPECT_EQ(GX_INVALID_WIDGET, status);

    /* Create string scroll wheel for testing. */
    gx_utility_rectangle_define(&size, 0, 0, 100, 100);
    status = _gxe_string_scroll_wheel_create_ext(&wheel, "test_wheel", NULL, 0, GX_NULL, GX_STYLE_ENABLED, 1024, &size, sizeof(GX_STRING_SCROLL_WHEEL));
    ...
    /* Set string list that contains invalid string. */
    status = _gxe_string_scroll_wheel_string_list_set_ext(&wheel, test_string_list_2, TEST_STRING_LIST_SIZE);
    EXPECT_EQ(GX_INVALID_STRING_LENGTH, status);

    /* Set string list with invalid string list count. */
    status = _gxe_string_scroll_wheel_string_list_set_ext(&wheel, test_string_list_2, -1);
    EXPECT_EQ(GX_INVALID_VALUE, status);

    /* Set null pointer string list. */
    status = _gxe_string_scroll_wheel_string_list_set_ext(&wheel, GX_NULL, 0);
    EXPECT_EQ(GX_SUCCESS, status);
    ...
    status = _gxe_string_scroll_wheel_string_list_set_ext(&wheel, test_valid_string_list, TEST_STRING_LIST_SIZE);
    EXPECT_EQ(GX_SUCCESS, status);
    ...
    status = _gxe_string_scroll_wheel_string_list_set(&wheel, test_valid_old_string_list, TEST_STRING_LIST_SIZE);
    EXPECT_EQ(GX_SYSTEM_MEMORY_ERROR, status);
#endif

    /* Set string id list. */
    status = _gxe_string_scroll_wheel_string_id_list_set(&wheel, test_string_id_list, TEST_STRING_LIST_SIZE);
    EXPECT_EQ(GX_SYSTEM_MEMORY_ERROR, status);
    ...
    status = _gxe_widget_delete((GX_WIDGET *)&wheel);
    EXPECT_EQ(GX_SUCCESS, status);
